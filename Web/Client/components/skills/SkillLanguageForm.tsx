...
export const SkillLanguageForm = (
	props: ISkillLanguageFormProps 
) => {
	const { 
		skillId,
		language,
	...
	const [initialValues, setInitialValues] = useState<ISkillLanguage>(language ?? {
		id: 0,
		locale: "",
		name: "New Skill Name",
	...
	<Formik<ISkillLanguage>
		...
		onSubmit={async (values, action) => {
			try {
				var res = await axios.post(`/api/skill/${skillId}/languages/${values.locale}`, values);
				if(res.status == 200) {
